...
import { Truck, MapPin, CheckCircle, Navigation, ExternalLink, Package, ShieldCheck } from 'lucide-react';
...
const DeliveryDashboard = () => {
  ...
  const mockAssigned = [
    { id: 'BOX-12948', customer: 'John Doe', address: '742 Evergreen Terrace', status: 'ready', distance: '3.2 km' }
  ];
    ...
    <div className="min-h-screen bg-bg-main text-white p-6 md:p-12 relative overflow-hidden">
       ...
       <div className="container max-w-4xl mx-auto relative z-10">
          ...
          <AnimatePresence mode="wait">
             {activeTab === 'pending' ? (
                <motion.div 
                  key="pending" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}
                  className="space-y-6"
                >
                   {mockAssigned.map(ord => (
                      <div key={ord.id} className="bg-secondary/40 p-10 rounded-3xl border border-white/5 shadow-2xl overflow-hidden relative group">
                            ...
                            <div className="space-y-6 flex-1">
                               ...
                               <div className="space-y-4">
                                  ...
                                  <div className="flex items-center gap-4 text-sm font-medium">
                                     ...
                                     <span className="text-xs uppercase tracking-widest font-bold">Estimated Path: {ord.distance}</span>
                                  </div>
                               </div>
                            </div>
                ...
                <motion.div 
                  key="completed" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
