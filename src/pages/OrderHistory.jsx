...
import { Truck, MapPin, Package, Clock, CheckCircle, ChevronRight, Home, Building, Plus } from 'lucide-react';
...
const OrderHistory = () => {
  ...
  const mockActiveOrders = [
    { id: 'BOX-12948', status: 'preparing', items: '2x Special Chicken Biryani', total: '$31.98', eta: '12-15 min' }
  ];

  const mockHistory = [
    { id: 'BOX-09284', date: 'Oct 12, 2026', total: '$25.99', items: 'Family Combo', status: 'delivered' }
  ];
    ...
    <div className="min-h-screen bg-bg-main text-white p-6 md:p-12 relative overflow-hidden">
       ...
       <div className="container max-w-5xl mx-auto relative z-10">
          ...
          <AnimatePresence mode="wait">
             {activeTab === 'active' && (
                ...
                >
                   {mockActiveOrders.map(order => (
                      <div key={order.id} className="bg-secondary/40 p-10 rounded-3xl border border-white/5 shadow-2xl overflow-hidden relative">
                         ...
                         <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-12 items-center">
                            <div className="space-y-6">
                               <div className="flex items-center gap-4">
                                  ...
                                  <div>
                                     <p className="text-[10px] font-bold text-text-muted uppercase tracking-widest leading-none mb-1">Order Identifier</p>
                                     <p className="text-xl font-bold font-heading">{order.id}</p>
                                  </div>
                               </div>
                               <div>
                                  <p className="text-[10px] font-bold text-text-muted uppercase tracking-widest leading-none mb-2">Artisan Status</p>
                                  <div className="flex items-center gap-2">
                                     <div className="w-2 h-2 rounded-full bg-primary animate-pulse" />
                                     <span className="text-sm font-bold text-primary uppercase">{order.status}</span>
                                  </div>
                               </div>
                            </div>
                            ...
                            <div className="space-y-4">
                               ...
                               <div className="flex items-center gap-4 text-sm font-medium">
                                  ...
                                  <span>Arrival in approx <span className="text-primary">{order.eta}</span></span>
                               </div>
                            </div>
                            ...
                            <div className="flex flex-col items-center justify-center gap-6">
                               <div className="w-40 h-40 rounded-full border-4 border-white/5 flex items-center justify-center shrink-0 p-4">
                                  <div className="w-full h-full rounded-full border-4 border-primary border-t-white/10 animate-spin flex items-center justify-center">
                                     <span className="text-xs font-black uppercase tracking-widest -rotate-90">Hot</span>
                                  </div>
                               </div>
             ...
             {activeTab === 'history' && (
                ...
                >
                   {mockHistory.map(order => (
                      <div key={order.id} className="flex items-center gap-8 bg-secondary/30 p-8 rounded-3xl border border-white/5 hover:border-white/10 transition-all cursor-pointer group">
                         ...
                         <div className="flex-1">
                            ...
                            <p className="text-xs text-text-muted uppercase tracking-widest">{order.date} • {order.id}</p>
                         </div>
                         <div className="text-right">
                            <p className="text-xl font-bold text-primary font-heading mb-1">{order.total}</p>
             ...
             {activeTab === 'addresses' && (
                <motion.div 
                  key="addresses" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
                  className="grid md:grid-cols-2 gap-6"
                >
                   {mockAddresses.map(addr => (
                      <div key={addr.id} className="bg-secondary/40 p-8 rounded-3xl border border-white/5 relative group">
